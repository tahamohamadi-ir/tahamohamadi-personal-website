...
const publicPreviewPath = computed(() => {
  ...
  return slug ? `/${selectedLocale.value}/${slug}` : null
})
...
<template>
  <q-page class="q-pa-md q-pa-lg-md">
    <div class="row items-center justify-between q-col-gutter-md q-mb-lg">
      <div class="col">
        ...
        <p class="text-body2 text-grey-8 q-mb-none">
          About and Research translations are edited independently.
        </p>
      </div>
      <div class="col-auto">
        <q-btn color="primary" label="Create page" @click="create" />
      </div>
    </div>
        ...
        <q-item v-for="item in items" :key="item.id" clickable @click="select(item)">
          <q-item-section>
            ...
            <q-item-label caption>
              {{ item.fa?.title || 'Missing translation' }} · {{ item.en?.title || 'Missing translation' }}
            </q-item-label>
          </q-item-section>
          <q-item-section side>
    ...
    <q-form class="q-mt-xl q-gutter-md" @submit.prevent="save">
      <h2 class="text-h6 q-my-none">{{ form.id ? 'Edit page' : 'Create page' }}</h2>
      <q-input v-model="form.pageKey" label="Page key" :disable="saving" />
      ...
      <q-input v-model="activeTranslation.canonicalPath" label="Canonical path" :disable="saving" />
