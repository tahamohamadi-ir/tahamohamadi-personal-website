...
function createForm(value = {}) {
  return {
    id: value.id ?? null, categoryKey: value.categoryKey ?? '', sortOrder: value.sortOrder ?? 0,
    active: value.active ?? true, version: value.version ?? null,
...
async function load(requestedPage = page.value) {
  ...
  error.value = null
  try {
    const response = await httpClient.get('/api/v1/admin/blog/categories', {
...
async function select(item) {
  error.value = null
  try {
    form.value = createForm((await httpClient.get(`/api/v1/admin/blog/categories/${item.id}`)).data)
...
function payload() {
  return {
    categoryKey: form.value.categoryKey, sortOrder: Number(form.value.sortOrder),
    fa: form.value.fa, en: form.value.en,
...
async function save() {
  ...
  try {
    ...
    const response = form.value.id
      ? await httpClient.put(`/api/v1/admin/blog/categories/${form.value.id}`, payload())
      : await httpClient.post('/api/v1/admin/blog/categories', payload())
...
async function deactivate() {
  ...
  try {
    await primeCsrfToken(httpClient)
    await httpClient.delete(`/api/v1/admin/blog/categories/${form.value.id}`, {
...
<template>
  <q-page class="admin-page">
    <header class="admin-page__header">
      <div>
        <h1 class="text-h4 q-my-none">{{ t('admin.blogTaxonomy.categories.title') }}</h1>
        <p class="admin-page__description">{{ t('admin.blogTaxonomy.categories.description') }}</p>
      </div>
      <q-btn color="primary" no-caps icon="add" :label="t('admin.blogTaxonomy.categories.create')" @click="create" />
    </header>
    <q-banner v-if="error" class="bg-red-1 text-negative" role="alert">
      {{ isVersionConflict(error) ? t('admin.blogTaxonomy.categories.conflict') : error.message }}
    </q-banner>
    ...
    <q-form class="admin-panel q-pa-md q-gutter-md" @submit.prevent="save">
      <div class="admin-page__header">
        <h2 class="text-h5 q-my-none">{{ form.id ? t('admin.blogTaxonomy.categories.edit') : t('admin.blogTaxonomy.categories.new') }}</h2>
        <span class="text-caption">{{ t('admin.blogTaxonomy.version', { version: form.version ?? t('admin.blogTaxonomy.newVersion') }) }}</span>
      </div>
      <q-input v-model="form.categoryKey" :label="t('admin.blogTaxonomy.categories.key')" :disable="saving" :error="Boolean(fieldErrors.categoryKey)" :error-message="fieldErrors.categoryKey" />
      <q-input v-model.number="form.sortOrder" type="number" min="0" :label="t('admin.blogTaxonomy.sortOrder')" :disable="saving" />
      ...
      <div class="admin-form-actions">
        <q-btn type="submit" color="primary" no-caps :loading="saving" :label="t('admin.blogTaxonomy.categories.save')" />
