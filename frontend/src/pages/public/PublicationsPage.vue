...
import PublicationList from 'src/components/public/PublicationList.vue'
...
function isEmptyPublications(value) {
...
const {
  ...
  api,
  load: (currentApi) => currentApi.listPublications(locale.value, {
    page: page.value,
    size: 20
  }),
  isEmpty: isEmptyPublications,
  initialData: props.initialData
})

const publications = computed(() => data.value?.items ?? [])
...
<template>
  <section class="collection-page tm-container">
    <h1 class="tm-page-title">{{ t('shell.navigation.publications') }}</h1>
    <PageState
      v-if="state"
      :state="state"
      @retry="retry"
