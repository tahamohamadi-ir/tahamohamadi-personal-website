...
describe('admin routing contract', () => {
  ...
  it('provides a responsive login form and an authenticated shell', () => {
    ...
    expect(loginSource).toContain('useI18n')
    expect(loginSource).toContain("t('admin.login.submit')")
    expect(loginSource).toContain('q-layout')
    expect(loginSource).toContain('q-page-container')
    expect(layoutSource).toContain('q-drawer')
    expect(layoutSource).toContain('data-testid="admin-navigation"')
    expect(layoutSource).toContain('logout')
    expect(layoutSource).toContain('noindex')
    expect(layoutSource).toContain('closeMobileNavigationAfterSelection')
    expect(layoutSource).toContain('$q.screen.lt.md')
    expect(layoutSource).toContain("t('admin.chrome.toggleNavigation')")
    expect(layoutSource).toContain('watch(')
    expect(layoutSource).toContain('drawerOpen.value = true')
    expect(layoutSource).not.toContain('class="lt-md"')
    expect(layoutSource).not.toContain('@click="drawerOpen = false"')
  })
