...
class AdminBlogLifecycleIntegrationTest {
    ...
    void enforcesLifecycleRulesSecurityCsrfAndOptimisticVersions() throws Exception {
        ...
        String id = createPost(admin, category.getId(), false, List.of(tag.getId()));
        ...
        String second = createPost(admin, category.getId(), true, List.of());
    ...
    private String createPost(AppUser admin, UUID categoryId, boolean seo, List<UUID> tagIds) throws Exception {
        org.springframework.test.web.servlet.ResultActions action = mvc.perform(post("/api/v1/admin/blog/posts").contentType(MediaType.APPLICATION_JSON)
                        .content(postPayload(categoryId, null, seo, tagIds)).with(adminUser(admin)).with(SecurityMockMvcRequestPostProcessors.csrf()))
    ...
    private long updatePost(AppUser admin, String id, UUID categoryId, long version, boolean seo) throws Exception {
        String response = mvc.perform(put("/api/v1/admin/blog/posts/{id}", id).contentType(MediaType.APPLICATION_JSON)
                        .content(postPayload(categoryId, version, seo, null)).with(adminUser(admin)).with(SecurityMockMvcRequestPostProcessors.csrf()))
                .andExpect(status().isOk()).andReturn().getResponse().getContentAsString();
        return ((Number) JsonPath.read(response, "$.version")).longValue();
    }

    private static String postPayload(UUID categoryId, Long version, boolean seo, List<UUID> tagIds) {
        ...
        return "{\"categoryId\":\"" + categoryId + "\",\"fa\":{\"title\":\"فا\",\"slug\":\"fa-" + suffix + "\",\"bodyMarkdown\":\"body\"" + seoFields + "},\"en\":{\"title\":\"En\",\"slug\":\"en-" + suffix + "\",\"bodyMarkdown\":\"body\"" + seoFields + "}" + versionField + tagsField + "}";
    }
