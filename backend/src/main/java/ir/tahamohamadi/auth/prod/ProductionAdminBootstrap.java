package ir.tahamohamadi.auth.prod;
...
@ConditionalOnProperty(
        name = "taha.bootstrap.admin.enabled",
        havingValue = "true"
)
...
public class ProductionAdminBootstrap implements ApplicationRunner {
    ...
    @Value("${TAHA_PROD_ADMIN_EMAIL:}")
    private String email;

    @Value("${TAHA_PROD_ADMIN_PASSWORD:}")
    private String password;

    @Override
    @Transactional
    public void run(ApplicationArguments args) {
        String configuredEmail = requireConfiguredValue(
                email,
                "TAHA_PROD_ADMIN_EMAIL"
        );

        String configuredPassword = requireConfiguredValue(
                password,
                "TAHA_PROD_ADMIN_PASSWORD"
        );

        Instant now = Instant.now();

        AppUser user = appUserRepository
                .findByNormalizedEmail(configuredEmail)
        ...
        Role adminRole = roleRepository
                .findActiveByCode(ADMIN_ROLE)
                .orElseThrow(() -> new IllegalStateException(
                        "The ADMIN role must exist before production bootstrap."
                ));
    ...
    private static String requireConfiguredValue(
        ...
        if (value == null || value.isBlank()) {
            throw new IllegalStateException(
                    variableName +
                    " is required when production admin bootstrap is enabled."
            );
        }
