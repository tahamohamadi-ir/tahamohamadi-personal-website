...
public class AdminBlogService {
    private final BlogPostRepository posts; private final BlogPostTranslationRepository translations; private final BlogCategoryRepository categories; private final TagRepository tags; private final BlogPostTagRepository postTags; private final MediaAssetRepository media; private final BlogPostMediaRepository postMedia;
    private final AuditEventRepository audit; private final ObjectMapper mapper; private final AuthenticatedAuditActor actor;

    public AdminBlogService(BlogPostRepository posts, BlogPostTranslationRepository translations, BlogCategoryRepository categories, TagRepository tags, BlogPostTagRepository postTags, MediaAssetRepository media, BlogPostMediaRepository postMedia, AuditEventRepository audit, ObjectMapper mapper, AuthenticatedAuditActor actor) {
        this.posts=posts; this.translations=translations; this.categories=categories; this.tags=tags; this.postTags=postTags; this.media=media; this.postMedia=postMedia; this.audit=audit; this.mapper=mapper; this.actor=actor;
    }
    ...
    public AdminBlogResponse create(AdminBlogCreateRequest request) {
        ...
        save(post,request.fa(),request.en()); replaceTags(post,request.tagIds()); replaceMedia(post,request.media()); posts.flush(); record("ADMIN_BLOG_POST_CREATED",post.getId()); return response(post);
    }
    ...
    public AdminBlogResponse update(UUID id,AdminBlogUpdateRequest request) {
        ...
        save(post,request.fa(),request.en()); replaceTags(post,request.tagIds()); replaceMedia(post,request.media()); posts.flush(); record("ADMIN_BLOG_POST_UPDATED",id); return response(post);
    }
