...
public class PublicContentController {
    ...
    @GetMapping("/posts") ResponseEntity<Map<String,Object>> posts(@PathVariable LanguageCode lang,@RequestParam(required=false) String q,@RequestParam(defaultValue="0") int page,@RequestParam(defaultValue="20") int size){check(page,size);return cached(Map.of("items",content.posts(lang,q,page,size),"page",page,"size",size));}
    ...
    private static void check(int page,int size){if(page<0||size<1||size>100)throw new IllegalArgumentException("Invalid pagination");}
}
