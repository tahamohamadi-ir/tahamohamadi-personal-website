...
public class PublicContactService {
    ...
    @Transactional public ContactSubmissionResponse submit(ContactSubmissionRequest request,String remoteAddress){limit(remoteAddress);Instant submittedAt=Instant.now();UUID id=UUID.randomUUID();messages.save(ContactMessage.submit(id,request.name(),request.email(),request.message(),request.language(),submittedAt));return new ContactSubmissionResponse(id,"RECEIVED",submittedAt);}
