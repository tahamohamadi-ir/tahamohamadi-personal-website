...
> این سند «لیست قابلیت محصول» است، نه مجوز تغییر فناوری. مخزن فعلی Java/Spring Boot و Vue/Quasar است. بخش Python + React فقط طرح مرجع برای یک محصول مستقل در آینده است؛ مهاجرت فناوری به‌تنهایی از این سند نتیجه نمی‌شود.
...
| حوزه | وضعیت فعلی |
|---|---|
| Composer صفحه | بلوک‌های `HERO`، `RICH_TEXT`، `MEDIA`، `MEDIA_TEXT`، `CALL_TO_ACTION`، `COLLECTION`، `SKILLS`، `RESUME`، `SOCIAL_LINKS`، `CONTACT`، `DIVIDER`، `SPACER`، `GALLERY`، `STATS` و `QUOTE` وجود دارند. |
| Collection | منبع‌های Blog، Portfolio و Publications برای blockهای collection موجود است. |
| Preview | preview صفحه با `no-store` و `noindex, nofollow` محافظت می‌شود. |
| انتشار | blog از lifecycle و scheduler بهره می‌برد. |
...
3. هر block قبل از ذخیره در server validate شود؛ React/Vue validation کافی نیست.
...
## 14. طرح معماری برای محصول مستقل Python + React

این بخش فقط در صورتی اجرا می‌شود که محصول مستقلی با Python + React تأیید شود.

```text
React Admin (CSR) ───────┐
React Public App (SSR) ──┼── Python API ── PostgreSQL
                         │        │
                         │        └── media storage
...
- `backend/src/main/java/ir/tahamohamadi/content/page/api/preview/PagePreviewController.java`
- `backend/src/main/java/ir/tahamohamadi/blog/post/BlogScheduledPublisher.java`
- `backend/src/main/java/ir/tahamohamadi/content/site/api/admin/AdminSiteSettingsController.java`
- `backend/src/main/java/ir/tahamohamadi/content/site/api/admin/AdminNavigationController.java`
- `docs/master-plan.md`
- `docs/tahamohamadi_site_cms_v2_gap_based_implementation_plan.md`
