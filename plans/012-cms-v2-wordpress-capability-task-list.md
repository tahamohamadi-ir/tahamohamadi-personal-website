...
**وضعیت:** آمادهٔ اجرا؛ هیچ آیتمی بدون گذر از Gate 0 وارد پیاده‌سازی نمی‌شود.
**مرجع محصول:** `docs/wordpress-capability-extraction-and-cms-reference.md`
**مرجع اجرای V2:** `docs/tahamohamadi_site_cms_v2_gap_based_implementation_plan.md`
**هدف:** تکمیل تدریجی تجربهٔ مدیریت محتوا، نه ساختن یک WordPress عمومی.
**محدودهٔ فناوری فعلی:** Spring Boot + PostgreSQL/Flyway + Vue 3/Quasar SSR.
**خارج از دامنه:** مهاجرت به Python/React، page builder آزاد، HTML/CSS/JS دلخواه و زنجیرهٔ افزونه‌های WordPress.
...
- [ ] تغییر schema فقط با Flyway migration افزایشی انجام شده است.
...
|---|---|---|
| Media Picker | `AdminMediaPickerModal.vue` دارای جست‌وجو، filter نوع، pagination، upload-in-flow و انتخاب asset فعال است. | VERIFY + یکپارچه‌سازی مصرف‌کننده‌ها |
| Media Library | `AdminMediaPage.vue` upload، metadata، orphan، archive و replace را نمایش می‌دهد. | VERIFY + coverage و QA lifecycle |
| Composer | `AdminPageBlockComposer.vue` section، block، inspector، duplicate، keyboard move، undo/redo، preview و autosave Draft دارد. | VERIFY + parity/test و تکمیل drag interaction در صورت نیاز |
| Preview Token | `AdminPagePreviewTokenController` و `PagePreviewTokenService` وجود دارند. | VERIFY امنیت و عدم نشت token |
| Page Revisions | `AdminPageRevisionService` snapshot و restore-as-draft دارد. | FIX ناسازگاری یک UI route + VERIFY restore |
...
| Translation Queue | `AdminTranslationQueuePage.vue` و statusهای Missing/Incomplete/Complete/Outdated وجود دارند. | VERIFY منطق و pagination/scope |
| Article Blocks | `ArticleBlockEditor.vue` و `articleDocument.js` برای blockهای پایه وجود دارند. | VERIFY + BUILD catalog و policy کامل |
...
- `backend/src/main/java/ir/tahamohamadi/content/page/api/admin/AdminPageController.java`
- `backend/src/main/java/ir/tahamohamadi/content/page/api/admin/AdminPageBlockController.java`
- `backend/src/main/java/ir/tahamohamadi/media/api/admin/AdminMediaController.java`
- `backend/src/main/java/ir/tahamohamadi/blog/post/api/admin/AdminBlogController.java`
- `frontend/src/services/httpClient.js` و serviceهای Admin/Media مرتبط
...
**نوع:** FIX
**یافتهٔ تأییدشده:** `AdminPageEditPage.vue` به مسیر `.../revisions/{id}/restore` درخواست می‌فرستد؛ controller موجود مسیر `.../restore-as-draft` را با `version` اجباری ارائه می‌کند. `AdminPagesPage.vue` از قرارداد درست استفاده می‌کند.

**فایل‌های هدف:**

- `frontend/src/pages/admin/AdminPageEditPage.vue`
- `backend/src/main/java/ir/tahamohamadi/content/page/api/admin/AdminPageController.java` (فقط برای تأیید؛ endpoint جدید نسازید مگر ADR جداگانه)
- تست frontend مناسب برای Page Edit
- `backend/src/test/java/ir/tahamohamadi/admin/AdminPageAuditAndConcurrencyIntegrationTest.java`
...
- [ ] تمام routeهای Admin Page را از `frontend/src/router/routes.js` با controllerها تطبیق دهید.
...
- `frontend/src/components/admin/AdminMediaPickerModal.vue`
- `frontend/src/components/admin/AdminMediaSelector.vue`
- `frontend/src/pages/admin/AdminMediaPage.vue`
...
- `frontend/src/components/admin/AdminPageBlockComposer.vue`
...
- [ ] PagePreviewController برای guest و token نامعتبر/منقضی negative test داشته باشد.
...
- `frontend/src/services/articleDocument.js`
- `frontend/src/components/admin/ArticleBlockEditor.vue`
- `frontend/src/components/content/ArticleDocumentContent.vue`
